...
def index(request):
    ...
    lista_Preguntas = Question.objects.order_by('-category')
...
def results(request, question_id):
    question = get_object_or_404(Question, pk=question_id)
    return render(request, 'polls/results.html', {'title':'Resultados de la pregunta:','question': question, 'acertado': acertado})

def vote(request, question_id):
    p = get_object_or_404(Question, pk=question_id)
    opciones = Choice.objects.filter(question_id = question_id)
    try:
        ...
        global acertado
        if p.choice_set.get(pk=request.POST['choice']).correcta:
            acertado
            acertado = 'Es correcta'
        else:
            acertado
            acertado = 'No es correcta'
...
def choice_add(request, question_id):
        ...
        if request.method =='POST':
            ...
            if form.is_valid():
                p = Choice.objects.filter(question_id = question_id)
                j = 0
                tieneCorrecta = False
                for i in p:
                    if p[j].correcta == 1:
                    ...
                    j = j + 1
                if len(p) < 4: 
                    if (not tieneCorrecta or (tieneCorrecta and not form['correcta'].value())):
                        ...
                        error = 'Ya se tiene una opción correcta'
                else:
                    error = 'No se pueden añadir más de 4 opciones'
        else: 
            form = ChoiceForm()
        #return render_to_response ('choice_new.html', {'form': form, 'poll_id': poll_id,}, context_instance = RequestContext(request),)
        return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form, 'error': error, 'question': question})
